...
const opendir = getDirectory().values
  ...
  async start(controller) { 
    for await (const file of await opendir(directory)) {
      file.isDirectory() && recursive 
        && openDirStream(`${file.path}/${file.name}`)
          .pipeTo(new WriteableStream(write(file){controller,enqueue(`${file.path}/${file.name}`)}))
        : controller.enqueue(`${file.path}/${file.name}`);
    };
  },
})
...
// dir.ent withFileTypes result of readdir(,{withFileType})
// Reflect.ownKeys(Filehandle)[1] === Symbol.for("type"); FileHandle[type] = 1; 
export const IFileSystemHandle = (name,kind,data) => {
  
  const handle = {
    getDirectory: kind === 'directory' && () => fs.readdirSync('./',{recursive:true,withFileTypes: true})
      .map((f)=>[f.name,f[Reflect.ownKeys(f)[1]]]),
    getFile: kind === 'file' && () => data || Readable.toWeb(fs.createReadStream(
...
export StorageManager = navigator ? navigator.storage : ({
  getDirectory: () => ['name',dataOrArrayOfFileSystemHandle]
});
